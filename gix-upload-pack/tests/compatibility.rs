...
fn compare_pack_contents(
    native_info: &PackInfo, 
    gix_info: &PackInfo
) -> Result<(), String> {
    ...
    if native_objects.len() != gix_objects.len() {
        return Err(format!(
            ...
            native_objects.len(), gix_objects.len()
        ));
    }
    
    // Create sets of object IDs for comparison
    let native_ids: std::collections::HashSet<_> = native_objects.iter()
        .map(|obj| &obj.id)
        .collect();
    let gix_ids: std::collections::HashSet<_> = gix_objects.iter()
        .map(|obj| &obj.id)
        .collect();
    ...
    if !missing_in_gix.is_empty() {
        return Err(format!(
            ...
            missing_in_gix.iter().take(5).map(|id| &id[..8]).collect::<Vec<_>>().join(", ")
        ));
    }
    ...
    if !missing_in_native.is_empty() {
        return Err(format!(
            ...
            missing_in_native.iter().take(5).map(|id| &id[..8]).collect::<Vec<_>>().join(", ")
        ));
    }
    ...
    // Create lookup maps
    let native_map: std::collections::HashMap<_, _> = native_objects.iter()
        .map(|obj| (&obj.id, obj))
        .collect();
    let gix_map: std::collections::HashMap<_, _> = gix_objects.iter()
        .map(|obj| (&obj.id, obj))
        .collect();
        ...
        if let (Some(native_obj), Some(gix_obj)) = (native_map.get(id), gix_map.get(id)) {
            if native_obj.obj_type != gix_obj.obj_type {
                ...
                if type_mismatches <= 3 {
                    println!("    ⚠️ Type mismatch for {}: native={}, gix={}", 
                        &id[..8], native_obj.obj_type, gix_obj.obj_type);
                }
            }
            ...
            if native_obj.size != gix_obj.size {
                ...
                if size_mismatches <= 3 {
                    println!("    ⚠️ Size mismatch for {}: native={}, gix={}", 
                        &id[..8], native_obj.size, gix_obj.size);
                }
            }
        }
    }
    ...
    println!("      Native: {}/{} objects are deltas ({:.1}%)", 
        native_deltas, native_objects.len(),
        (native_deltas as f64 / native_objects.len() as f64) * 100.0);
    println!("      Gix:    {}/{} objects are deltas ({:.1}%)", 
        gix_deltas, gix_objects.len(),
...
fn parse_verify_pack_output(output: &str) -> Result<Vec<PackObject>, String> {
    ...
    for line in output.lines() {
        ...
        if parts.len() >= 5 {
            ...
            let size = parts[2].parse::<u64>().map_err(|_| format!("Invalid size: {}", parts[2]))?;
            let packed_size = parts[3].parse::<u64>().map_err(|_| format!("Invalid packed size: {}", parts[3]))?;
            let offset = parts[4].parse::<u64>().map_err(|_| format!("Invalid offset: {}", parts[4]))?;
...
impl ClientPacketFixture {
    ...
    fn modify_v2_fixture(
        ...
        while pos < base_data.len() {
            ...
            let packet_len = u16::from_str_radix(len_str, 16)
                .map_err(|_| "Invalid hex packet length")? as usize;
...
fn run_gix_upload_pack(input: &[u8], repo_path: &str, protocol_version: Option<&str>) -> Result<Vec<u8>, Box<dyn std::error::Error>> {
...
fn compare_advertise_refs_outputs(
    native: &[u8], 
    gix: &[u8], 
) -> Result<(), String> {
    if native == gix {
    ...
    if native_caps_normalized != gix_caps_normalized {
        differences.push(format!("Capabilities differ (excluding agent):\n  Native: {:?}\n  Gix: {:?}", 
            native_caps_normalized, gix_caps_normalized));
    } else if native_caps != gix_caps {
        println!("  ~ Agent strings differ (acceptable): native has {:?}, gix has {:?}",
            native_caps.split_whitespace().find(|cap| cap.starts_with("agent=")).unwrap_or("no-agent"),
            gix_caps.split_whitespace().find(|cap| cap.starts_with("agent=")).unwrap_or("no-agent"));
    }
    ...
    for (i, (native_line, gix_line)) in native_lines.iter().zip(gix_lines.iter()).enumerate() {
        ...
        let is_agent_only_diff = if native_line.contains('\0') && gix_line.contains('\0') {
            ...
            let native_content = if native_line.len() > 4 { &native_line[4..] } else { native_line };
        ...
        if !is_agent_only_diff && native_line != gix_line {
            differences.push(format!("Line {} differs:\n  Native: {:?}\n  Gix: {:?}", 
                i + 1, native_line, gix_line));
            significant_differences += 1;
            if significant_differences >= 3 {  // Limit output for readability
    ...
    let error_msg = format!("Advertise-refs outputs do not match exactly:\n  {}", 
        differences.join("\n  "));
...
fn test_advertise_refs_protocol(protocol_version: Option<&str>) -> Result<(), Box<dyn std::error::Error>> {
    ...
    compare_advertise_refs_outputs(&native_output, &gix_output)
        .map_err(|e| e.into())
}
...
fn compare_outputs(native: &[u8], gix: &[u8], fixture: &ClientPacketFixture) -> Result<(), String> {
    ...
    println!("    Native: NAK={}, Progress={}, Pack={} bytes", 
    ...
    println!("    Gix:    NAK={}, Progress={}, Pack={} bytes",
    ...
    match (&native_analysis.nak_response, &gix_analysis.nak_response) {
        (Some(n_nak), Some(g_nak)) => {
            if n_nak != g_nak {
                protocol_issues.push(format!("NAK responses differ: native={:?}, gix={:?}", 
                    String::from_utf8_lossy(n_nak), String::from_utf8_lossy(g_nak)));
            }
        }
    ...
    let fixture_has_sideband = fixture.features.iter().any(|f| 
        f.contains("side-band") || f.contains("sideband")); // v2 protocol always uses sideband
    ...
    if let (Some(native_pack), Some(gix_pack)) = (&native_analysis.pack_data, &gix_analysis.pack_data) {
        ...
        let save_pack_result = validate_pack_data_from_memory(
            native_pack, 
            gix_pack, 
            &fixture.name, 
            pack_dir
        ...
        match save_pack_result {
            Ok((native_pack_info, gix_pack_info)) => {
                ...
                println!("    Native: {} ({} objects)", native_pack_info.path, native_pack_info.object_count);
                println!("    Gix:    {} ({} objects)", gix_pack_info.path, gix_pack_info.object_count);
        ...
        if native_pack == gix_pack {
            ...
            let size_diff_percent = (native_pack.len() as f64 - gix_pack.len() as f64).abs() / native_pack.len() as f64 * 100.0;
            
            if size_diff_percent > 1.0 {
                pack_differences.push(format!(
                    "Pack size difference too large: {:.2}% (native: {}, gix: {})",
                    size_diff_percent, native_pack.len(), gix_pack.len()
                ));
            } else {
                println!("  ~ Pack sizes differ within tolerance: {:.2}% (native: {}, gix: {})",
                         size_diff_percent, native_pack.len(), gix_pack.len());
            }
            ...
            if size_diff_percent > 1.0 {
                ...
                if let Some(diff_pos) = first_diff {
                    println!("  ~ Pack data differs at byte {} (within tolerance): native=0x{:02x} gix=0x{:02x}",
                             diff_pos, native_pack[diff_pos], gix_pack[diff_pos]);
                }
            }
        }
    ...
    if fixture_has_sideband && native_analysis.progress_messages != gix_analysis.progress_messages {
        ...
        let native_has_enumeration = native_analysis.progress_messages.iter()
            .any(|msg| msg.contains("Enumerating objects"));
        let gix_has_enumeration = gix_analysis.progress_messages.iter()
            .any(|msg| msg.contains("Enumerating objects"));
            
        if native_has_enumeration && !gix_has_enumeration {
            message_differences.push("Missing 'Enumerating objects' phase - should precede 'Counting objects'".to_string());
        }
        
        // Check counting format differences
        let native_counting = native_analysis.progress_messages.iter()
            .find(|msg| msg.contains("Counting objects:   0%"));
        let gix_counting = gix_analysis.progress_messages.iter()
            .find(|msg| msg.contains("Counting objects: 0%"));
            
        if native_counting.is_some() && gix_counting.is_some() {
            message_differences.push("Progress format differs: native uses '   0%' (3 spaces), gix uses '0%' (no spaces)".to_string());
            message_differences.push("Progress line endings: native uses \\r (carriage return), gix uses \\n (newline)".to_string());
        }
...
fn test_compatibility_with_fixtures() {
    ...
    if message_failures > 0 {
        println!("  - Message format failures: {} (should match native Git exactly)", message_failures);
    }
    if pack_failures > 0 {
    ...
    if protocol_failures > 0 {
        panic!("{} critical protocol compliance failures! Fix protocol implementation.", protocol_failures);
    }
    
    if message_failures > passed / 2 {  // If more than half have message format issues
        panic!("{} message format failures (>{} allowed). Progress messages should match native Git format.", message_failures, passed / 2);
    }
    
    if pack_failures > (passed + failed) * 3 / 4 {  // If more than 75% have pack issues, that's concerning
        panic!("{} pack data failures (>{} allowed). Check pack generation.", pack_failures, (passed + failed) * 3 / 4);
    }
